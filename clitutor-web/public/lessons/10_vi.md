...
>
> **Lab performance note:** In this browser VM, prefer `vi` for snappier
> editing. `vim` is installed and available, but may feel slower.
...
vim filename.txt         # available, but heavier/slower in this lab VM
```

<!-- exercise
id: ex01
title: Launch vimtutor
xp: 10
difficulty: 1
sandbox_setup: null
...
expected: "vimtutor"
hints:
  - "Start the built-in tutorial directly; it opens a guided drill inside Vim."
  - "Spend focused time moving around the sample file, following the comments, and exit with `:q` when you are ready."
  - "After you leave vimtutor, run `command -v vimtutor` so this exercise can confirm completion."
-->
### Exercise 1: Launch vimtutor
Run `vimtutor` and spend focused time following it end-to-end; when you exit, type `command -v vimtutor` so the lesson can confirm the binary exists.
...
expected: practice.txt::third line
hints:
  - "Create a file with multiple lines for practicing navigation."
  - "Use printf with \\n for newlines."
  - "Type: `printf 'first line\\nsecond line\\nthird line\\nfourth line\\nfifth line\\n' > practice.txt`"
-->
### Exercise 2: Create a multi-line file
Create `practice.txt` with five lines of text (including the text `third line`).
...
The file `edit_me.txt` has three lines. Remove the second line (`delete this line`) and display the result.
...
<!-- exercise
id: ex04
title: Search and replace with sed (vi-style)
xp: 15
difficulty: 2
sandbox_setup:
...
hints:
  ...
  - "Type: `sed -i 's/quick/slow/g' animals.txt && cat animals.txt`"
-->
### Exercise 4: Search and replace (vi-style)
Replace all occurrences of `quick` with `slow` in `animals.txt` (using sed, which uses the same syntax as vi's substitute command).
